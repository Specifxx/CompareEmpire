...
export const metadata: Metadata = {
  title: "Free One Piece Price Widget — Embed Live Card Prices",
  description:
    "Add a free, auto-updating One Piece card price badge to your website, blog, store or Discord. It always shows the cheapest live price across stores and links back to the full comparison.",
  alternates: { canonical: "/widgets" },
  openGraph: {
    title: "Free One Piece Card Price Widget",
    description:
      "Embed a live, auto-updating One Piece card price on any site. Always shows the cheapest price across stores.",
...
export default async function WidgetsPage() {
  // AU-baseline examples (the page is cached/ISR); the embed itself takes a
  // ?market= param so a user can price it in their own market.
  const examples = await getPopularCards(3, DEFAULT_COUNTRY).catch(() => []);

  return (
    <div className="flex flex-col gap-10">
      {/* Hero */}
      <section className="card-surface overflow-hidden">
        <div className="rounded-lg bg-gradient-to-br from-brand-600/20 via-ink-850 to-gold/10 px-6 py-12 text-center">
          ...
          <h1 className="mx-auto max-w-2xl text-2xl font-extrabold text-white sm:text-4xl">
            Embed a live One Piece card price on your site
          </h1>
      ...
      <section>
        ...
        {examples.length > 0 ? (
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {examples.map((c: (typeof examples)[number]) => (
              <EmbedSnippet
                key={c.id}
                title={cardDisplayName(c.name, c)}
                src={`${SITE_URL}/embed/card/${c.slug ?? c.id}?market=AU`}
              />
            ))}
          </div>
        ) : (
      ...
      <section id="how" className="card-surface p-6">
        ...
        <ol className="mt-3 max-w-2xl list-decimal space-y-2 pl-5 text-sm leading-relaxed text-slate-300">
          <li>
            Copy one of the <code className="text-slate-200">&lt;iframe&gt;</code> snippets above (or
            build one for any card — the URL is <code className="text-slate-200">/embed/card/&lt;slug&gt;</code>).
          </li>
          <li>Paste it into your page&apos;s HTML.</li>
          <li>
        ...
        <div className="mt-5 rounded-xl border border-ink-700 bg-ink-900 p-4">
          ...
          <ul className="mt-2 space-y-1 text-xs leading-relaxed text-slate-400">
            ...
            <li><code className="text-slate-200">?market=</code> — set the market it prices in: <code className="text-slate-200">AU</code>, <code className="text-slate-200">NZ</code>, <code className="text-slate-200">US</code> or <code className="text-slate-200">GB</code>.</li>
          </ul>
        </div>
      </section>
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            mainEntity: [
              {
                "@type": "Question",
                name: "Is the One Piece price widget free?",
                acceptedAnswer: {
                  "@type": "Answer",
