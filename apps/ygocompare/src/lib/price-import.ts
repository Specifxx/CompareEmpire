// Reusable price-import engine. Pulls Yu-Gi-Oh! singles from AU Shopify stores'
...
// Word tokens for Yu-Gi-Oh! name/set matching. Strips only generic noise (condition,
...
// Auto-discover a store's Yu-Gi-Oh! singles collections from its Shopify sitemap,
...
export async function discoverYugiohCollections(base: string): Promise<string[]> {
  ...
  for (const sm of sitemaps.slice(0, 8)) {
    ...
    for (const m of xml.matchAll(/\/collections\/([^<\/?#"]+)/g)) {
      const h = m[1];
      // Require a Yu-Gi-Oh! signal in the handle, and skip sealed/accessory
      // collections and image URLs. "singles" alone qualifies a Yu-Gi-Oh!-named store.
...
// Build a Yu-Gi-Oh! title→cardId resolver over a set of cards. Yu-Gi-Oh! store titles
...
export async function importPrices(): Promise<ImportSummary> {
  ...
  async function processStore(store: RetailerInfo): Promise<void> {
    const cc = store.country ?? "AU";
    // Auto-discover the store's Yu-Gi-Oh! collections from its sitemap (authoritative).
    // Only fall back to handles configured in retailers.ts if discovery finds nothing,
    // and never scrape a non-Yu-Gi-Oh! (e.g. legacy "riftbound") handle.
    let handles = await discoverYugiohCollections(store.base);
  ...
  // The Yu-Gi-Oh! catalogue (20k+ cards) is far larger than the daily Browse quota,
