...
export async function generateMetadata({
  ...
  if (!store) notFound();
  const title = `${store.name} — One Piece Card Prices & Store Profile`;
  const description = `Is ${store.name} cheaper? See how many One Piece cards ${store.name} currently has at the lowest tracked price, its shipping policy, and every in-stock listing OPCompare compares.`;
    ...
    <div className="flex flex-col gap-8">
      ...
      <div>
        ...
        <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-400">
          {store.name} is one of the {COUNTRIES[country].label} One Piece Card Game stores OPCompare compares.{" "}
          {stats.cheapestCount > 0 ? (
            ...
            <>OPCompare tracks {stats.totalInStock} in-stock listings from {store.name}.</>
          )}{" "}
