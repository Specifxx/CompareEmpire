...
export function EmbedSnippet({ src, title, width = 340, height = 150 }: { src: string; title: string; width?: number; height?: number }) {
  ...
  const snippet = `<iframe src="${src}" width="${width}" height="${height}" style="border:0;overflow:hidden" loading="lazy" title="DexCompare live price — ${title}"></iframe>`;
  ...
  return (
    <div className="card-surface p-4">
      ...
      <iframe
        ...
        loading="lazy"
        title={`DexCompare live price — ${title}`}
      />
