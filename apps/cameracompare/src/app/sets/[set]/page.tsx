...
export async function generateMetadata({ params }: { params: { set: string } }): Promise<Metadata> {
  ...
  return {
    ...
    keywords: [
      `camera ${set.name}`,
      `camera ${set.name} prices`,
      `camera ${set.name} card list`,
      `${set.name} card prices`,
      `cheapest camera ${set.name} cards`,
      `camera ${set.name} value`,
    ],
    alternates: { canonical: `/sets/${set.slug}` },
    openGraph: { title: `${title} | CameraCompare`, description, url: `${SITE_URL}/sets/${set.slug}` },
  };
}
...
export default async function SetPage({ params }: { params: { set: string } }) {
  ...
  const collection = {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    name: `camera ${set.name} Card Prices & List`,
    url: `${SITE_URL}/sets/${set.slug}`,
    description: `Live prices for every camera ${set.name} card.`,
    isPartOf: { "@type": "WebSite", name: "CameraCompare", url: SITE_URL },
  };
