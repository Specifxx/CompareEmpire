...
export async function generateMetadata({ params }: { params: { set: string } }): Promise<Metadata> {
  ...
  const title = `Magic ${set.name} Card Prices, Values & Full Card List`;
  const description = `Every Magic ${set.name} card with live prices compared across stores — find the cheapest ${set.name} singles. Full ${set.name} card list and values, updated daily.`;
  return {
    title: { absolute: `${title} | MTGCompare` },
    description,
    keywords: [
      `Magic ${set.name}`,
      `Magic ${set.name} prices`,
      `Magic ${set.name} card list`,
      `${set.name} card prices`,
      `cheapest Magic ${set.name} cards`,
      `Magic ${set.name} value`,
    ],
    alternates: { canonical: `/sets/${set.slug}` },
    openGraph: { title: `${title} | MTGCompare`, description, url: `${SITE_URL}/sets/${set.slug}` },
  };
}
...
export default async function SetPage({ params }: { params: { set: string } }) {
  ...
  const collection = {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    name: `Magic ${set.name} Card Prices & List`,
    url: `${SITE_URL}/sets/${set.slug}`,
    description: `Live prices for every Magic ${set.name} card.`,
    isPartOf: { "@type": "WebSite", name: "MTGCompare", url: SITE_URL },
  };
    ...
    <div className="flex flex-col gap-8">
      ...
      <div>
        ...
        <h1 className="text-2xl font-extrabold text-white sm:text-3xl">
          Magic {set.name} — card prices &amp; full list
        </h1>
        <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-400">
          {set.comingSoon ? (
            <>Magic <strong className="text-slate-200">{set.name}</strong> isn&apos;t out yet. This page will list every {set.name} card with live prices the moment it releases — check back soon.</>
          ) : (
            <>Browse all {cards.length} Magic <strong className="text-slate-200">{set.name}</strong> cards and compare live prices across stores to find the cheapest singles. {priced.toLocaleString()} cards are priced right now, updated daily — switch your country at the top to see local prices.</>
          )}
        </p>
      </div>
      ...
      <section>
        <h2 className="mb-3 text-lg font-bold text-white">Other Magic sets</h2>
      ...
      {!set.comingSoon && (
        <section className="card-surface p-6">
          <h2 className="text-xl font-extrabold text-white">About Magic {set.name}</h2>
          <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-400">
            {set.name} is a set in the Magic Trading Card Game. MTGCompare tracks live prices
