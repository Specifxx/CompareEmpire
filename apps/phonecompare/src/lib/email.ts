...
export async function sendEmail(to: string, subject: string, html: string): Promise<boolean> {
  ...
  // Send from the verified phonecompare.com domain by default so Resend allows
  ...
  const from = process.env.EMAIL_FROM ?? `${SITE_NAME} <noreply@phonecompare.com>`;
  try {
...
function layout(heading: string, body: string, cta: { label: string; url: string }): string {
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#0b0e14;padding:32px 0"><tr><td align="center">
    <table role="presentation" width="480" cellpadding="0" cellspacing="0" style="background:#131a26;border:1px solid #233047;border-radius:16px">
      ...
      <tr><td style="padding:16px 32px 26px;border-top:1px solid #233047;font-size:12px;color:#6b7585">PhoneCompare · phone price comparison.<br/>If you didn't request this, you can safely ignore this email.</td></tr>
...
export async function sendVerificationEmail(to: string, token: string): Promise<boolean> {
  return sendEmail(
    to,
    "Confirm your PhoneCompare email",
    layout("Confirm your email", "Thanks for signing up — confirm your email address to finish setting up your PhoneCompare account.", {
      label: "Confirm email",
...
export async function sendPasswordResetEmail(to: string, token: string): Promise<boolean> {
  return sendEmail(
    to,
    "Reset your PhoneCompare password",
    layout("Reset your password", "We received a request to reset your PhoneCompare password. This link expires in 1 hour.", {
