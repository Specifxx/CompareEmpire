...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  ...
  const title = `${card.name} (${card.setCode} ${card.collectorNumber}) — phone price in ${info.place}`;
  const description = `Compare live ${info.adjective} prices for ${card.name}, phone ${card.setName} ${card.collectorNumber}${price}. Find the cheapest store to buy this phone in ${info.place}.`;
...
export default async function CardPage({ params }: { params: { id: string } }) {
  ...
  // phones come in both standard and FOIL finishes (same collector number,
  ...
  const jsonLd = {
    ...
    name: card.name,
    category: "Phone",
    description: `${card.name} — phone ${card.setName} (${card.setCode}) ${card.collectorNumber}. Compare ${info.adjective} prices.`,
        ...
        <div className="min-w-0">
          <div className="card-surface p-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
              ...
              {card.might != null && <Metric label="RAM" value={`${card.might}GB`} />}
              <Metric label="Form" value={card.domain} />
            </div>
          </div>
          ...
          <div className="card-surface mt-6 overflow-hidden">
            ...
            <p className="border-t border-ink-800 p-3 text-center text-[11px] text-slate-600">
              Prices are collected from public store listings and may change. PhoneCompare
