// Chart primitives for the daily Market Wrap, ported from RiftCompare's daily
// market report. Everything renders on the server as plain SVG/HTML — no client
// JS, no chart library — so the diagrams cost nothing on mobile and print like
// a financial-press chart.
//
// Color policy (validated against the ink-900 surface): polarity colors only —
// emerald = up, red = down, slate = flat — matching the site-wide up/down
// convention. Deutan separation of the pair sits in the "floor" band, which is
// acceptable ONLY with secondary encoding, so direction is never color-alone
// anywhere these render: DeltaChip always carries a ▲/▼/■ glyph, DivergingBar
// encodes sign by side of a centre axis, and BreadthBar labels its counts.
...
export const TREND = {
  up: "#34d399", // emerald-400 (site-wide "up")
  down: "#f87171", // red-400 (site-wide "down")
...
// ── The hero chart: index level over the recent sessions ───────────────────────
export function IndexAreaChart({ series, id, ariaLabel }: { series: SeriesPoint[]; id: string; ariaLabel: string }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full" role="img" aria-label={ariaLabel}>
      ...
      {/* closing dot + halo — the chart's focal point */}
...
// ── Tiny sparkline (hub hero, market rows) ──────────────────────────────────────
...
// ── Signed-percentage chip (▲ +0.42% / ▼ −0.18%) — never color-alone ───────────
export function DeltaChip({ pct, dp = 2 }: { pct: number | null; dp?: number }) {
  ...
  const cls = pct > 0 ? "text-emerald-400" : pct < 0 ? "text-red-400" : "text-slate-400";
  return (
    <span className={`num text-xs font-bold ${cls}`}>
      {pct > 0 ? "▲" : pct < 0 ? "▼" : "■"} {pct > 0 ? "+" : ""}
      {pct.toFixed(dp)}%
    </span>
  );
...
// ── Diverging horizontal bar (markets, movers) ──────────────────────────────────
...
export function DivergingBar({ pct, scale }: { pct: number | null; scale: number }) {
  const p = pct ?? 0;
  const half = scale > 0 ? Math.min(Math.abs(p) / scale, 1) * 50 : 0;
  return (
    <div className="relative h-4 flex-1 overflow-hidden rounded bg-ink-800">
      ...
      <div
        className={`absolute inset-y-1 rounded-sm ${p >= 0 ? "bg-emerald-400/80" : "bg-red-400/80"}`}
...
// ── Market breadth: advancers vs decliners as a labeled stacked bar ─────────────
export function BreadthBar({ rose, fell, flat }: { rose: number; fell: number; flat: number }) {
  ...
  return (
    <div>
      <div className="flex h-3 gap-0.5 overflow-hidden rounded-full bg-ink-800">
        {rose > 0 && <div className="bg-emerald-400/90" style={{ width: w(rose) }} />}
        {flat > 0 && <div className="bg-slate-500/70" style={{ width: w(flat) }} />}
        {fell > 0 && <div className="bg-red-400/90" style={{ width: w(fell) }} />}
      </div>
      <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-400">
        <span>
          <span className="mr-1 inline-block h-2 w-2 rounded-full bg-emerald-400/90" />
          {rose.toLocaleString()} advanced
        </span>
        <span>
          <span className="mr-1 inline-block h-2 w-2 rounded-full bg-slate-500/70" />
          {flat.toLocaleString()} unchanged
        </span>
        <span>
          <span className="mr-1 inline-block h-2 w-2 rounded-full bg-red-400/90" />
          {fell.toLocaleString()} declined
        </span>
      </div>
    </div>
  );
