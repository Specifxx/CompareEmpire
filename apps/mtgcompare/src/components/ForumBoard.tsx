    ...
    <div>
      <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-400">
            Buy, sell and talk Magic: The Gathering with other players in {myCountryName}. List several cards in one
      ...
      <div className="mb-5 flex items-start gap-3 rounded-xl border border-ink-700 bg-ink-900/60 p-3.5">
        ...
        <p className="text-xs leading-relaxed text-slate-400">
          ...
          tracked payment and postage. {SITE_NAME} is not a party to any trade between members.
        </p>
      </div>
      ...
      {showForm && (
        <form onSubmit={submit} className="card-surface mb-6 space-y-3 p-4">
          ...
          <input
            value={form.title}
            onChange={set("title")}
            placeholder={isDiscussion ? "Title (required) — e.g. Best budget mono-red deck?" : "Title (required) — e.g. Selling my Bloomburrow doubles"}
            className="input"
            required
          />
      ...
      <p className="mt-6 text-center text-[11px] text-slate-600">
        {SITE_NAME} hosts these community listings but is not a party to any trade. Deal carefully and meet/pay safely.
      </p>
