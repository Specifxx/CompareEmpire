...
export const metadata: Metadata = {
  title: "Magic: The Gathering Top Meta Decks & Build Cost (Australia)",
  description:
    "Ready-made Magic: The Gathering decklists, each priced live across Australian stores. See what it costs to build a deck and where to buy every card.",
    ...
    <div className="flex flex-col gap-10">
      <div>
        <div className="mb-4">
          ...
          <p className="mt-1 text-sm text-slate-400">
            Real top-finishing tournament decklists from the current Magic: The Gathering metagame — each
            priced live across {info.adjective} stores so you can see what it costs to build and
      ...
      {beginner.length > 0 && (
        <div>
          <div className="mb-4">
            ...
            <p className="mt-1 text-sm text-slate-400">
              New to Magic: The Gathering? These budget-friendly decks are built mostly from cheap commons
      ...
      {decks.length > 0 && (
        <p className="text-center text-[11px] text-slate-600">
          Decklists are real tournament results and change with the metagame. Build cost uses each
          card&apos;s cheapest in-stock {info.adjective} price and may span multiple stores.
        </p>
      )}
    </div>
  );
  ...
  return (
    <div className="card-surface grid place-items-center p-16 text-center text-slate-400">
      <div className="max-w-md">
        <p className="text-lg font-semibold text-white">No meta decks yet</p>
        <p className="mt-1 text-sm">
          We&apos;re still building out the {SITE_NAME} meta deck library. In the meantime, paste your
          own list into the{" "}
          <Link href="/deck" className="text-brand-400 hover:underline">
            Deck Builder
          </Link>{" "}
          to price a whole deck at once.
        </p>
      </div>
    </div>
  );
