...
export function NavDropdown({
  ...
  label: string;
  items: { href: string; label: string; desc?: string; badge?: string }[];
}) {
  ...
  return (
    <div ref={ref} className="relative">
      <button
        onClick={() => setOpen((o) => !o)}
        aria-expanded={open}
        className="flex items-center gap-1 rounded-lg px-2.5 py-2 text-sm font-medium text-slate-200 hover:bg-ink-800 hover:text-white"
      >
        {label}
          ...
          {items.map((it) => (
            ...
            >
              <div className="flex items-center gap-2 text-sm font-medium text-white">
                {it.label}
                {it.badge && <span className="rounded-full bg-gold/15 px-1.5 py-0.5 text-[10px] font-bold uppercase text-gold">{it.badge}</span>}
              </div>
