...
function tagline(country: string, contextual: boolean): string {
  ...
  if (country === "US") return "Millions of One Piece listings";
...
export function EbayAd({
  ...
  const q = query ? `one piece ${query}` : "one piece tcg cards";
  const href = searchUrl(q, country);
  const label = query ? `Find ${query.length > 28 ? query.slice(0, 27) + "…" : query} on eBay` : "Shop One Piece cards on eBay";
