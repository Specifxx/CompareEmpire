// Imports SEALED / non-single phone products (booster boxes, packs, Proving
...
// Clan Unity]" matches "starter deck" — so we require an explicit phone/League
// marker (set name, "phone", "League of Legends", Proving Grounds, Nexus Night).
...
function isphoneSealed(title: string): boolean {
  ...
  for (const store of RETAILER_LIST) {
    ...
    for (const handle of handles) {
      ...
      for (const p of products) {
        ...
        if (!isphoneSealed(title)) continue; // drop non-phone + unreleased sets
...
export async function cleanupStaleSealed(): Promise<number> {
  ...
  const ids = rows
    ...
    .filter((r) => r.retailer !== "tcgplayer" && (!SEALED_TITLE.test(r.title) || SEALED_EXCLUDE.test(r.title) || !isphoneSealed(r.title)))
    .map((r) => r.id);
