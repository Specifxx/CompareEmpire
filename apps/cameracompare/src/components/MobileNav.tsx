  ...
  return (
    <div className="lg:hidden">
      <button
        ...
        aria-label="Menu"
        aria-expanded={open}
        className="grid h-9 w-9 place-items-center rounded-lg text-slate-700 hover:bg-slate-50"
      >
      ...
      {open && (
        <>
          ...
          <div className="absolute right-2 top-14 z-50 w-56 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl">
            {INTL_ENABLED && (
                  ...
                  {COUNTRY_LIST.map((c) => (
                    <button
                      ...
                      className={`flex items-center gap-2.5 rounded-lg px-2.5 py-2 text-left ${
                        c.code === country ? "bg-slate-50" : "hover:bg-slate-50"
                      }`}
                    >
              ...
              {LINKS.map((l) => (
                <li key={l.href}>
                  <Link
                    ...
                    className="block px-4 py-3 text-sm font-medium text-slate-700 hover:bg-slate-50 hover:text-slate-900"
                  >
                    {l.label}
                  </Link>
                </li>
