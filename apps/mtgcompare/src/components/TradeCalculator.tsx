...
interface TradeCard {
  ...
  // Printing credentials, so same-name cards (base vs promo vs alt-art) are
...
export function TradeCalculator() {
  ...
  // Each side has its OWN value % — so each trader can discount their cards (e.g.
  // value them at 90%) independently. The cash difference is then the gap between
  ...
  // Restore an in-progress trade (handy at meets — survives a refresh).
  useEffect(() => {
    try {
      ...
      <div className="sticky bottom-3 z-20 mt-4">
        <div className="card-surface border-ink-600 bg-ink-900/95 p-4 shadow-2xl backdrop-blur">
          ...
          <p className="mt-2 text-[11px] text-slate-600">
            Values start from MTGCompare&apos;s lowest live price (tap a price to type your own or pick a store).
    ...
    if (next && rows == null) {
      ...
      try {
        ...
        const list = ((data.retailerPrices ?? []) as any[])
          // Real, buyable stores only — the market-guide reference never ranks here.
          .filter((p) => p.inStock && p.country === country && !String(p.retailer ?? "").startsWith("marketguide"))
          .map((p) => ({
