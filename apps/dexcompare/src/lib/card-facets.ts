...
//
// NOTE on scope: the sprint brief's "printing" facet included 1st Edition,
...
import { speciesSlugify } from "./pokemon-species";
...
const TYPE_LABELS: Record<string, string> = { "Pokémon": "Pokémon", Trainer: "Trainer", Energy: "Energy" };

export async function resolveFacet(kind: string, slug: string): Promise<FacetValue | null> {
  if (kind === "type") {
    const entry = Object.entries(TYPE_LABELS).find(([name]) => speciesSlugify(name) === slug);
...
export async function listFacetValues(kind: FacetKind): Promise<FacetValue[]> {
  if (kind === "type") {
    return Object.entries(TYPE_LABELS).map(([name, label]) => ({ kind, slug: speciesSlugify(name), label, where: { type: name } }));
  }
  if (kind === "printing") {
  ...
  if (kind === "rarity") {
    ...
    try {
      ...
      return grouped
        ...
        .map((g) => ({ kind, slug: speciesSlugify(g.rarity), label: g.rarity, where: { rarity: g.rarity } }));
    } catch {
      return [];
    }
  }
