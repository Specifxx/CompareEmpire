...
function tagline(country: string, contextual: boolean): string {
  ...
  if (country === "US") return "Millions of Yu-Gi-Oh! listings";
...
export function EbayAd({
  ...
  const q = query ? `yugioh ${query}` : "yugioh tcg cards";
  const href = searchUrl(q, country);
  const label = query ? `Find ${query.length > 28 ? query.slice(0, 27) + "…" : query} on eBay` : "Shop Yu-Gi-Oh! cards on eBay";
