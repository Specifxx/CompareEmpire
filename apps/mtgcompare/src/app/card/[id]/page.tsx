...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  ...
  const title = `${card.name} (${card.setCode} ${card.collectorNumber}) — Magic Card Price`;
  const description = `Compare live prices for ${card.name}, Magic ${card.setName} ${card.collectorNumber}, across stores in Australia, New Zealand, the US and the UK — find the cheapest place to buy.`;
...
export default async function CardPage({ params }: { params: { id: string } }) {
  ...
  // Magic cards span a big condition range (NM → damaged). Build the cheapest
  ...
  // (NB: many Magic chase cards exist ONLY as foil — TCGplayer marks them
  ...
  const ebaySearchHref = ebaySearchUrl(`magic ${card.name} ${card.collectorNumber.split("/")[0]}`, country);
  ...
  const jsonLd = {
    ...
    description: `${card.name} — Magic ${card.setName} (${card.setCode}) ${card.collectorNumber}. Compare ${info.adjective} prices.`,
