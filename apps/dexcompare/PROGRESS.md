...
- SEO: dedicated OG share image for `/card-value` — added
  `src/app/card-value/opengraph-image.tsx`, a `next/og` `ImageResponse` card in the same
  on-brand red-accent style as the browse/sets/decks/restock/guides/blog OG images,
  showing the "Pokémon card value checker" headline, the current-year "Free Price
  Guide" kicker (mirrors the dynamic-year logic already in the page's
  `generateMetadata`), and the "live market value / real store prices" hooks.
  `/card-value` is a high-intent evergreen lander (the biggest query family in the
  hobby — "pokemon card value"/"price checker") that previously fell back to the
  generic site-wide OG card whenever shared · GSC-TARGETS.md was read first this run:
  the "low-CTR, high-impression" table is empty ("not enough impressions yet"); the
  only "striking distance" entry is `/` (84 impr, pos 10.7), which already has its
  trimmed title/description from a prior run — nothing new to change there. The top-
  queries table shows "pokemon card price checker website" / "pokemon card value
  checker" ranking at pos 90 with only 1 impression each — too thin a signal (a single
  impression) to justify a metadata rewrite when `/card-value`'s title/keywords
  already target that exact phrase; chasing it further would be guessing, not
  data-driven. Picked BACKLOG SEO queue item #8's explicitly-named next target
  (`/card-value` OG coverage) instead · verified with a clean rebuild (file present,
  same fake `DATABASE_URL`) that the exact same 8 pre-existing pages fail identically
  (`/`, `/trending`, `/deals`, `/most-valuable`, `/restock`, `/blog/market-wrap`,
  sitemap buckets 1/2) — this change adds none, and `/card-value/opengraph-image`
  itself compiles and generates cleanly; `tsc --noEmit` clean · (this commit)
- SEO: dedicated OG share image for `/browse` — added `src/app/browse/opengraph-image.tsx`, a `next/og` `ImageResponse` card in the same on-brand red-accent style as the sets/decks/restock/guides/blog OG images, showing a "Browse every Pokémon card" headline plus a real static stat (`{POKEMON_SETS.length} sets`) and the AU/NZ/US/UK markets line. Reads only the static `POKEMON_SETS` catalogue (no DB call), so it's unaffected by this sandbox's Prisma-auth build limitation. `/browse` is the highest-traffic page on the site and previously fell back to the generic purple/lightning-bolt default whenever a filtered/search link got shared · GSC-TARGETS.md was read first this run: no page qualifies for the "low-CTR, high-impression" table yet ("not enough impressions"); the only "striking distance" entry is `/` at pos 10.7 (84 impr), which already has its trimmed title/description from prior runs — nothing new to change there. Picked BACKLOG SEO queue item #8's explicitly-named next target instead (`/browse` OG coverage, first of the remaining `/browse, /card-value, /most-valuable, /sealed` list) · verified with a clean rebuild (file present vs. moved out, same fake `DATABASE_URL`) that the exact same 8 pre-existing pages fail identically (`/`, `/trending`, `/deals`, `/most-valuable`, `/restock`, `/blog/market-wrap`, sitemap buckets 1/2) — this change adds none, and `/browse/opengraph-image` itself compiles and generates cleanly; `tsc --noEmit` clean · (this commit)
- SEO: FAQPage + visible FAQ section on `/market` (the DexCompare Index) — 4 Q&As ("What is the Index?", "How is it calculated?", "Is it free to cite?", "How often does it update?") added as a `<details>` accordion matching the existing `/stores`/`/browse`/`/sealed`/`/sets/[set]`/`/deals` pattern, plus `FAQPage` JSON-LD merged into the page's existing `BreadcrumbList`+`Dataset` script tag · GSC-TARGETS.md this run flagged the same two targets as prior runs — `/card/base4-57-poliwhirl` (27 impr / 0.0% CTR / pos 2.7) and homepage striking-distance (96 impr / pos 8.8) — both already verified fixed in source (`b18360f` rewrote the exact `/card/[id]` title the poliwhirl code comment cites; homepage title/description were trimmed in an earlier run), so no fresh GSC-driven rewrite target existed; per BACKLOG's own note this is expected lag while Google re-crawls. Picked BACKLOG SEO queue item #7 instead: an audit of every FAQ-bearing page found `/market` was the one substantive, already-ranking "linkable asset" page still missing FAQ coverage (`/decks`, `/decks/[slug]`, `/restock/[slug]` also lack it but have far less prose/real-question content to draw from) · `tsc --noEmit` clean, `next build` clean (`/market` route compiles, same pre-existing 8-page fake-`DATABASE_URL` Prisma-auth build limitation, unaffected by this change) · (this commit)
