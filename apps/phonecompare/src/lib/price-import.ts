// Reusable price-import engine. Pulls phones from AU Shopify stores'
...
// Auto-discover a store's phones collections from its Shopify sitemap,
...
async function discoverphoneCollections(base: string): Promise<string[]> {
...
export async function importPrices(): Promise<ImportSummary> {
  ...
  for (const store of RETAILER_LIST) {
    const cc = store.country ?? "AU";
    // Auto-discover the store's phone collections; fall back to any handles
    // configured explicitly in retailers.ts.
    let handles = await discoverphoneCollections(store.base);
