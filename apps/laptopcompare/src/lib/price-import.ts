// Reusable price-import engine. Pulls laptops from AU Shopify stores'
...
// Auto-discover a store's laptops collections from its Shopify sitemap,
...
async function discoverlaptopCollections(base: string): Promise<string[]> {
...
export async function importPrices(): Promise<ImportSummary> {
  ...
  for (const store of RETAILER_LIST) {
    const cc = store.country ?? "AU";
    // Auto-discover the store's laptop collections; fall back to any handles
    // configured explicitly in retailers.ts.
    let handles = await discoverlaptopCollections(store.base);
