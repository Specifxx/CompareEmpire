...
export async function generateMetadata({
  ...
  const title = `${facet.label} One Piece Cards — Prices & Full List`;
  const description = `Browse every ${facet.label} One Piece card OPCompare tracks — compare live prices across stores and find the cheapest ${facet.label.toLowerCase()} singles.`;
...
export default async function FacetPage({
  ...
  const collection = {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    name: `${facet.label} One Piece Card Prices`,
    url: `${SITE_URL}/cards/${params.facet}/${params.value}`,
    description: `Every tracked ${facet.label} One Piece card with live prices.`,
    isPartOf: { "@type": "WebSite", name: "OPCompare", url: SITE_URL },
  };

  const faqs = [
    {
      q: `What is a ${facet.label} One Piece card?`,
      a:
        facet.kind === "type"
          ? `"${facet.label}" is one of the three One Piece Card Game card supertypes — One Piece, Trainer and Energy. OPCompare tracks ${total} ${facet.label} cards.`
          : facet.kind === "rarity"
          ? `"${facet.label}" is a One Piece Card Game rarity tier. OPCompare tracks ${total} cards printed at ${facet.label} rarity across every set.`
          : facet.kind === "era"
          ? `${facet.label} is a One Piece Card Game set series/era. OPCompare tracks ${total} cards across every ${facet.label} set.`
          : `A ${facet.label} printing is a special, non-standard-pack One Piece card. OPCompare tracks ${total} Promo cards.`,
    },
    {
    ...
    <div className="flex flex-col gap-8">
      ...
      <div>
        ...
        <h1 className="text-2xl font-extrabold text-white sm:text-3xl">
          {facet.label} One Piece cards — prices &amp; full list
        </h1>
        <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-400">
          OPCompare tracks <strong className="text-slate-200">{total}</strong> {facet.label} {kindLabel === "Card type" ? "" : "One Piece "}cards
          {priced > 0 ? (
            <>
