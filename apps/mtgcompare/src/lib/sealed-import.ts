// Imports SEALED / non-single Magic products (booster boxes, elite trainer
...
// Magic marker: the word "Magic", or a detected Magic set name in the title.
const POKEMON_HINT = /pok[eé]mon|\bpkmn\b/i;

function isMagicSealed(title: string): boolean {
  return POKEMON_HINT.test(title) || detectSetMeta(title) != null;
}

// A sealed product title looks like one of these (Magic sealed lines).
const SEALED_TITLE =
...
// Discover Magic collections (including sealed-only ones, which the singles
// importer skips) so we can find boxes/packs/ETBs wherever the store files them.
// We match on Magic-ish handles plus the generic sealed buckets (booster-boxes,
// elite-trainer-box, sealed, …) so a store that files sealed under a game-agnostic
// collection is still covered — the per-product Magic check happens downstream.
...
// Classify a sealed product into a specific Magic product type. Shared by store
...
// Map a TCGplayer setName to our Magic set code via the catalogue (null for
  ...
  for (const store of RETAILER_LIST) {
    ...
    for (const handle of handles) {
      ...
      for (const p of products) {
        ...
        if (!isMagicSealed(title)) continue; // drop non-Magic + unreleased sets
...
export async function cleanupStaleSealed(): Promise<number> {
  ...
  const ids = rows
    ...
    .filter((r) => r.retailer !== "tcgplayer" && (!SEALED_TITLE.test(r.title) || SEALED_EXCLUDE.test(r.title) || !isMagicSealed(r.title)))
    .map((r) => r.id);
...
// Magic sealed set detection: longest set name that appears in the product
...
// Magic product-type display order (chase SKUs first).
