// Imports SEALED / non-single laptop products (booster boxes, packs, Proving
...
// Clan Unity]" matches "starter deck" — so we require an explicit laptop/League
// marker (set name, "laptop", "League of Legends", Proving Grounds, Nexus Night).
...
function islaptopSealed(title: string): boolean {
  ...
  for (const store of RETAILER_LIST) {
    ...
    for (const handle of handles) {
      ...
      for (const p of products) {
        ...
        if (!islaptopSealed(title)) continue; // drop non-laptop + unreleased sets
...
export async function cleanupStaleSealed(): Promise<number> {
  ...
  const ids = rows
    ...
    .filter((r) => r.retailer !== "tcgplayer" && (!SEALED_TITLE.test(r.title) || SEALED_EXCLUDE.test(r.title) || !islaptopSealed(r.title)))
    .map((r) => r.id);
