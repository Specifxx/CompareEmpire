...
// Heart toggle for a single card. `icon` is the small overlay button for tiles;
// `full` is a labelled button for the card page.
export function WishlistButton({
  ...
  useEffect(() => {
    ...
    const h = (e: Event) => {
      ...
      if (changed == null || changed === cardId) setOn(isWishlisted(cardId));
    };
  ...
  function click(e: React.MouseEvent) {
    // The button often sits inside a tile's <Link> — don't navigate on toggle.
    e.preventDefault();
    e.stopPropagation();
