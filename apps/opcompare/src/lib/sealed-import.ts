// Imports SEALED / non-single One Piece products (booster boxes, elite trainer
...
// Discover One Piece collections (including sealed-only ones, which the singles
// importer skips) so we can find boxes/packs/ETBs wherever the store files them.
// We match on One Piece-ish handles plus the generic sealed buckets (booster-boxes,
// elite-trainer-box, sealed, …) so a store that files sealed under a game-agnostic
// collection is still covered — the per-product One Piece check happens downstream.
...
// Classify a sealed product into a specific One Piece product type. Shared by store
...
// Map a TCGplayer setName to our One Piece set code via the catalogue (null for
  ...
  for (const store of RETAILER_LIST) {
    ...
    for (const handle of handles) {
      ...
      for (const p of products) {
        ...
        if (!isOnePieceSealed(title)) continue; // drop non-One Piece + unreleased sets
...
export async function cleanupStaleSealed(): Promise<number> {
  ...
  const ids = rows
    ...
    .filter((r) => r.retailer !== "tcgplayer" && (!SEALED_TITLE.test(r.title) || SEALED_EXCLUDE.test(r.title) || !isOnePieceSealed(r.title)))
    .map((r) => r.id);
...
// One Piece sealed set detection: longest set name that appears in the product
...
// One Piece product-type display order (chase SKUs first).
