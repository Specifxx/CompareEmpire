// Imports SEALED / non-single camera products (booster boxes, packs, Proving
...
// Clan Unity]" matches "starter deck" — so we require an explicit camera/League
// marker (set name, "camera", "League of Legends", Proving Grounds, Nexus Night).
...
function iscameraSealed(title: string): boolean {
  ...
  for (const store of RETAILER_LIST) {
    ...
    for (const handle of handles) {
      ...
      for (const p of products) {
        ...
        if (!iscameraSealed(title)) continue; // drop non-camera + unreleased sets
...
export async function cleanupStaleSealed(): Promise<number> {
  ...
  const ids = rows
    ...
    .filter((r) => r.retailer !== "tcgplayer" && (!SEALED_TITLE.test(r.title) || SEALED_EXCLUDE.test(r.title) || !iscameraSealed(r.title)))
    .map((r) => r.id);
