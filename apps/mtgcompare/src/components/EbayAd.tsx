...
function tagline(country: string, contextual: boolean): string {
  ...
  if (country === "US") return "Millions of Magic listings";
...
export function EbayAd({
  ...
  const q = query ? `magic ${query}` : "magic tcg cards";
  const href = searchUrl(q, country);
  const label = query ? `Find ${query.length > 28 ? query.slice(0, 27) + "…" : query} on eBay` : "Shop Magic cards on eBay";
