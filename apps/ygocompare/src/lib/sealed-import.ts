// Imports SEALED / non-single Yu-Gi-Oh! products (booster boxes, elite trainer
...
// Yu-Gi-Oh! marker: the word "Yu-Gi-Oh!", or a detected Yu-Gi-Oh! set name in the title.
const POKEMON_HINT = /pok[eé]mon|\bpkmn\b/i;

function isYugiohSealed(title: string): boolean {
  return POKEMON_HINT.test(title) || detectSetMeta(title) != null;
}

// A sealed product title looks like one of these (Yu-Gi-Oh! sealed lines).
const SEALED_TITLE =
...
// Discover Yu-Gi-Oh! collections (including sealed-only ones, which the singles
// importer skips) so we can find boxes/packs/ETBs wherever the store files them.
// We match on Yu-Gi-Oh!-ish handles plus the generic sealed buckets (booster-boxes,
// elite-trainer-box, sealed, …) so a store that files sealed under a game-agnostic
// collection is still covered — the per-product Yu-Gi-Oh! check happens downstream.
...
// Classify a sealed product into a specific Yu-Gi-Oh! product type. Shared by store
...
// Map a TCGplayer setName to our Yu-Gi-Oh! set code via the catalogue (null for
  ...
  for (const store of RETAILER_LIST) {
    ...
    for (const handle of handles) {
      ...
      for (const p of products) {
        ...
        if (!isYugiohSealed(title)) continue; // drop non-Yu-Gi-Oh! + unreleased sets
...
export async function cleanupStaleSealed(): Promise<number> {
  ...
  const ids = rows
    ...
    .filter((r) => r.retailer !== "tcgplayer" && (!SEALED_TITLE.test(r.title) || SEALED_EXCLUDE.test(r.title) || !isYugiohSealed(r.title)))
    .map((r) => r.id);
...
// Yu-Gi-Oh! sealed set detection: longest set name that appears in the product
...
// Yu-Gi-Oh! product-type display order (chase SKUs first).
