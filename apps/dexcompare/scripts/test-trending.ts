...
async function main() {
  ...
  const anyCard = await prisma.priceHistory.findFirst({ select: { cardId: true } });
  if (anyCard) {
    const h = await prisma.priceHistory.findMany({
...
main().catch((e) => { console.error("TRENDING QUERY FAILED:", e); process.exit(1); }).finally(() => prisma.$disconnect());
