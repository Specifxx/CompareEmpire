...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  ...
  const title = `${card.name} (${card.setCode} ${card.collectorNumber}) — Yu-Gi-Oh! Card Price`;
  const description = `Compare live prices for ${card.name}, Yu-Gi-Oh! ${card.setName} ${card.collectorNumber}, across stores in Australia, New Zealand, the US and the UK — find the cheapest place to buy.`;
...
export default async function CardPage({ params }: { params: { id: string } }) {
  ...
  // Yu-Gi-Oh! cards span a big condition range (NM → damaged). Build the cheapest
  ...
  // (NB: many Yu-Gi-Oh! chase cards exist ONLY as foil — TCGplayer marks them
  ...
  const ebaySearchHref = ebaySearchUrl(`yugioh ${card.name} ${card.collectorNumber.split("/")[0]}`, country);
  ...
  const jsonLd = {
    ...
    description: `${card.name} — Yu-Gi-Oh! ${card.setName} (${card.setCode}) ${card.collectorNumber}. Compare ${info.adjective} prices.`,
        ...
        <div className="min-w-0">
          ...
          <div className="card-surface mt-6 overflow-hidden">
            ...
            <p className="border-t border-ink-800 p-3 text-center text-[11px] text-slate-600">
              Prices are collected from public store listings and may change. YGOCompare
