// Imports SEALED / non-single car products (booster boxes, packs, Proving
...
// Clan Unity]" matches "starter deck" — so we require an explicit car/League
// marker (set name, "car", "League of Legends", Proving Grounds, Nexus Night).
...
function iscarSealed(title: string): boolean {
  ...
  for (const store of RETAILER_LIST) {
    ...
    for (const handle of handles) {
      ...
      for (const p of products) {
        ...
        if (!iscarSealed(title)) continue; // drop non-car + unreleased sets
...
export async function cleanupStaleSealed(): Promise<number> {
  ...
  const ids = rows
    ...
    .filter((r) => r.retailer !== "tcgplayer" && (!SEALED_TITLE.test(r.title) || SEALED_EXCLUDE.test(r.title) || !iscarSealed(r.title)))
    .map((r) => r.id);
