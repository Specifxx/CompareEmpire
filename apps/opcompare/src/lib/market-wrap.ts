...
// The Daily Market Wrap — an automated, AFR-style report on how the One Piece
...
async function computeWrap(day: string): Promise<MarketWrapData | null> {
  ...
  const headline = `OPCompare Global Index ${verbFor(globalD1)}${Math.abs(globalD1) < 0.05 ? "" : ` ${Math.abs(globalD1).toFixed(2)}%`}`;
  const insightBit = insight
    ? ` ${COUNTRIES[insight.country].place} moved hardest at ${fmtPct(insight.d1)}.`
    : "";
  const lede =
    `One Piece singles ${globalD1 >= 0.05 ? "rose" : globalD1 <= -0.05 ? "fell" : "held steady"} ${
      Math.abs(globalD1) < 0.05 ? "" : `${Math.abs(globalD1).toFixed(2)}% `
    }across the ${withData.length} markets OPCompare tracks, with ${marketsUp} market${marketsUp === 1 ? "" : "s"} up and ${marketsDown} down on the day.` +
    insightBit;
