// Reusable price-import engine. Pulls Magic singles from AU Shopify stores'
...
// Word tokens for Magic name/set matching. Strips only generic noise (condition,
...
export async function discoverMagicCollections(base: string): Promise<string[]> {
  ...
  for (const sm of sitemaps.slice(0, 8)) {
    ...
    for (const m of xml.matchAll(/\/collections\/([^<\/?#"]+)/g)) {
      const h = m[1];
      // Require a Magic signal in the handle, and skip sealed/accessory
      // collections and image URLs. "singles" alone qualifies a Magic-named store.
...
// Build a Magic title→cardId resolver over a set of cards. Magic store titles
...
export async function importPrices(): Promise<ImportSummary> {
  ...
  async function processStore(store: RetailerInfo): Promise<void> {
    const cc = store.country ?? "AU";
    // Auto-discover the store's Magic collections from its sitemap (authoritative).
    // Only fall back to handles configured in retailers.ts if discovery finds nothing,
    // and never scrape a non-Magic (e.g. legacy "riftbound") handle.
    let handles = await discoverMagicCollections(store.base);
  ...
  // The Magic catalogue (20k+ cards) is far larger than the daily Browse quota,
