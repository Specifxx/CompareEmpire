// Reusable price-import engine. Pulls cameras from AU Shopify stores'
...
// Auto-discover a store's cameras collections from its Shopify sitemap,
...
async function discovercameraCollections(base: string): Promise<string[]> {
...
export async function importPrices(): Promise<ImportSummary> {
  ...
  for (const store of RETAILER_LIST) {
    const cc = store.country ?? "AU";
    // Auto-discover the store's camera collections; fall back to any handles
    // configured explicitly in retailers.ts.
    let handles = await discovercameraCollections(store.base);
