...
export interface RetailerInfo {
  ...
  collections: string[]; // Shopify collection handles holding Yu-Gi-Oh! singles
...
export const RETAILERS: Record<string, RetailerInfo> = {
  ...
  // Additional Australian Yu-Gi-Oh! stores. Collections are auto-discovered from each
  // store's Shopify sitemap (Yu-Gi-Oh!-singles handles), so only the domain is needed.
  collectiblemadness: {
  ...
  // More local Australian Yu-Gi-Oh! singles stores (Shopify; collections auto-discovered
  ...
  // Additional US Yu-Gi-Oh! Shopify stores (collections auto-discovered from each
  ...
  // Additional US Yu-Gi-Oh! Shopify stores (round 2). Domains verified Shopify via
  // /collections paths or .myshopify.com; collections auto-discovered from each
  // store's sitemap, with a Yu-Gi-Oh!-singles handle as fallback. Each is validated
  ...
  // sitemap, with an explicit Yu-Gi-Oh!-singles handle as a fallback. Shipping figures
  // are GBP estimates.
  titancards: {
    key: "titancards",
    name: "Titan Cards",
  ...
  // Additional UK Yu-Gi-Oh! Shopify stores (round 2). Each validated by
