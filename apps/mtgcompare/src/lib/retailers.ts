...
export interface RetailerInfo {
  ...
  collections: string[]; // Shopify collection handles holding Magic singles
...
export const RETAILERS: Record<string, RetailerInfo> = {
  ...
  // Additional Australian Magic stores. Collections are auto-discovered from each
  // store's Shopify sitemap (Magic-singles handles), so only the domain is needed.
  collectiblemadness: {
  ...
  // More local Australian Magic singles stores (Shopify; collections auto-discovered
  ...
  // Additional US Magic Shopify stores (collections auto-discovered from each
  ...
  // Additional US Magic Shopify stores (round 2). Domains verified Shopify via
  // /collections paths or .myshopify.com; collections auto-discovered from each
  // store's sitemap, with a Magic-singles handle as fallback. Each is validated
  ...
  // sitemap, with an explicit Magic-singles handle as a fallback. Shipping figures
  // are GBP estimates.
  titancards: {
    key: "titancards",
    name: "Titan Cards",
  ...
  // Additional UK Magic Shopify stores (round 2). Each validated by
