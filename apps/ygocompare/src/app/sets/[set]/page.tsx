...
export async function generateMetadata({ params }: { params: { set: string } }): Promise<Metadata> {
  ...
  const title = `Yu-Gi-Oh! ${set.name} Card Prices, Values & Full Card List`;
  const description = `Every Yu-Gi-Oh! ${set.name} card with live prices compared across stores — find the cheapest ${set.name} singles. Full ${set.name} card list and values, updated daily.`;
  return {
    title: { absolute: `${title} | YGOCompare` },
    description,
    keywords: [
      `Yu-Gi-Oh! ${set.name}`,
      `Yu-Gi-Oh! ${set.name} prices`,
      `Yu-Gi-Oh! ${set.name} card list`,
      `${set.name} card prices`,
      `cheapest Yu-Gi-Oh! ${set.name} cards`,
      `Yu-Gi-Oh! ${set.name} value`,
    ],
    alternates: { canonical: `/sets/${set.slug}` },
    openGraph: { title: `${title} | YGOCompare`, description, url: `${SITE_URL}/sets/${set.slug}` },
  };
}
...
export default async function SetPage({ params }: { params: { set: string } }) {
  ...
  const collection = {
    "@context": "https://schema.org",
    "@type": "CollectionPage",
    name: `Yu-Gi-Oh! ${set.name} Card Prices & List`,
    url: `${SITE_URL}/sets/${set.slug}`,
    description: `Live prices for every Yu-Gi-Oh! ${set.name} card.`,
    isPartOf: { "@type": "WebSite", name: "YGOCompare", url: SITE_URL },
  };
    ...
    <div className="flex flex-col gap-8">
      ...
      <div>
        ...
        <h1 className="text-2xl font-extrabold text-white sm:text-3xl">
          Yu-Gi-Oh! {set.name} — card prices &amp; full list
        </h1>
        <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-400">
          {set.comingSoon ? (
            <>Yu-Gi-Oh! <strong className="text-slate-200">{set.name}</strong> isn&apos;t out yet. This page will list every {set.name} card with live prices the moment it releases — check back soon.</>
          ) : (
            <>Browse all {cards.length} Yu-Gi-Oh! <strong className="text-slate-200">{set.name}</strong> cards and compare live prices across stores to find the cheapest singles. {priced.toLocaleString()} cards are priced right now, updated daily — switch your country at the top to see local prices.</>
          )}
        </p>
      </div>
      ...
      <section>
        <h2 className="mb-3 text-lg font-bold text-white">Other Yu-Gi-Oh! sets</h2>
      ...
      {!set.comingSoon && (
        <section className="card-surface p-6">
          <h2 className="text-xl font-extrabold text-white">About Yu-Gi-Oh! {set.name}</h2>
          <p className="mt-2 max-w-3xl text-sm leading-relaxed text-slate-400">
            {set.name} is a set in the Yu-Gi-Oh! Trading Card Game. YGOCompare tracks live prices
