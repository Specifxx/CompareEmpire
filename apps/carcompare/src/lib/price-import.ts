// Reusable price-import engine. Pulls cars from AU Shopify stores'
...
// Auto-discover a store's cars collections from its Shopify sitemap,
...
async function discovercarCollections(base: string): Promise<string[]> {
...
export async function importPrices(): Promise<ImportSummary> {
  ...
  for (const store of RETAILER_LIST) {
    const cc = store.country ?? "AU";
    // Auto-discover the store's car collections; fall back to any handles
    // configured explicitly in retailers.ts.
    let handles = await discovercarCollections(store.base);
