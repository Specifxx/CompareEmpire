...
export const metadata: Metadata = {
  title: { absolute: "The MTGCompare Index — Magic Card Market Tracker | MTGCompare" },
  description:
    "One number for the health of the Magic: The Gathering singles market. The MTGCompare Index tracks the most valuable cards as a daily base-100 index per market, with a global composite, breadth, movers and sealed supply. Updated daily, free to cite.",
...
export default async function MarketPage({ searchParams }: { searchParams: { scope?: string } }) {
  ...
  const breadcrumbLd = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "MTGCompare Index", item: `${SITE_URL}/market` },
    ],
  };
  const datasetLd = startDay
    ? {
        "@context": "https://schema.org",
        "@type": "Dataset",
        name: "The MTGCompare Index",
        description: `Daily base-100 price index of the ${data.basketSize} most valuable Magic: The Gathering cards per market, from live store prices. Base 100 on ${startDay}.`,
        url: `${SITE_URL}/market`,
        creator: { "@type": "Organization", name: "MTGCompare", url: SITE_URL },
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <nav className="mb-3 flex items-center gap-1.5 text-xs text-slate-500" aria-label="Breadcrumb">
        ...
        <span>/</span>
        <span className="text-slate-300">MTGCompare Index</span>
      </nav>

      <header className="mb-5">
        <h1 className="font-display text-3xl font-extrabold text-white">📈 The MTGCompare Index</h1>
        <p className="mt-1 max-w-2xl text-slate-400">
          The Magic singles market in one number: a base-100 index of the {data.basketSize || "top"} most
      ...
      <section id="cite" className="card-surface mt-8 space-y-3 p-5 text-sm leading-relaxed text-slate-400">
        ...
        <p>
          The MTGCompare Index tracks what the {data.basketSize || "top"} most valuable in-scope Magic
        ...
        <p className="text-slate-300">
          ...
          &ldquo;the MTGCompare Index&rdquo; with a link to this page. The{" "}
