...
async function main() {
  console.log(`Loaded ${LAPTOPS.length} laptops.`);
  ...
  await prisma.user.create({
    data: { email: "demo@laptopcompare.com", passwordHash, displayName: "PowerUser", balanceCents: 0, isAdmin: true },
  });
  console.log("Created demo account: demo@laptopcompare.com / password123");

  let images: Record<string, string> = {};
  try {
    images = JSON.parse(readFileSync(join(process.cwd(), "prisma", "laptop-images.json"), "utf8"));
    console.log(`Loaded ${Object.keys(images).length} laptop images.`);
  } catch {
    console.warn("laptop-images.json not found — run scripts/fetch-laptop-images.ts. Using placeholders.");
  }

  const cardRows = LAPTOPS.map((l) => {
    const key = `${l.brand}-${l.model}`.toLowerCase();
    const img = images[key] ?? null;
    const usLow = cents(l.baseUsd * 100 * between(0.9, 1.0));
    const auLow = cents(usLow * USD_TO_AUD * between(0.98, 1.08));
    const gbLow = cents(usLow * USD_TO_GBP * between(0.98, 1.08));
    return {
      externalId: `${l.brand}-${l.model}`.toLowerCase(),
      slug: `${l.brand}-${l.model}`.toLowerCase().replace(/[^a-z0-9]+/g, "-"),
      name: l.name,
      nameNormalized: normalizeSearch(l.name),
      setCode: l.brand,
      setName: l.name.split(" ")[0],
      collectorNumber: `${l.year}`,
      domain: l.size,
      type: l.os,
      rarity: l.tier,
      might: l.ram, // reuse `might` to surface RAM (GB)
      tags: `${l.year}, ${l.os}, ${l.size}, ${l.ram}GB RAM`,
      description: `${l.name} — ${l.size}, ${l.os}, ${l.ram}GB RAM (${l.year}).`,
      imageUrl: img,
      imageThumbUrl: img,
  ...
  console.log(`Created ${cardRows.length} laptops.`);
  ...
  const auRetailers = [
    ...
    { key: "centrecom", name: "Centre Com", url: (q: string) => `https://www.centrecom.com.au/catalogsearch/result/?q=${q}`, spread: [0.95, 1.04] as [number, number] },
    { key: "scorptec", name: "Scorptec", url: (q: string) => `https://www.scorptec.com.au/search?q=${q}`, spread: [0.95, 1.05] as [number, number] },
  ];
  const usRetailers = [
    ...
    { key: "newegg", name: "Newegg", url: (q: string) => `https://www.newegg.com/p/pl?d=${q}`, spread: [0.96, 1.06] as [number, number] },
  ];
  const gbRetailers = [
