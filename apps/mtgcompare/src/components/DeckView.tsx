...
// legends imported as "Ragavan - Starter").
function cleanName(name: string): string {
  return name.replace(/\s*-\s*Starter$/i, "");
}

// Decklist display groups, in order. The featured card is shown above.
// "Main Deck" folds the Creature/Spell/Land cards together; the underlying
// section keys stay flexible so any sourced list shape works.
const GROUPS: { title: string; sections: string[]; muted?: boolean }[] = [
  { title: "Featured", sections: ["champion", "legend", "featured", "commander"] },
  { title: "Creatures", sections: ["creature", "pokemon", "unit", "planeswalker"] },
  { title: "Spells", sections: ["spell", "trainer", "gear", "supporter", "item", "stadium", "instant", "sorcery", "artifact", "enchantment"] },
  { title: "Lands", sections: ["land", "energy", "rune", "battlefield"] },
  { title: "Sideboard", sections: ["sideboard"], muted: true },
];
...
export function DeckView({ deck, builderHref }: { deck: ResolvedDeck; builderHref: string }) {
  ...
  // The big image on the left follows whichever card you hover (defaults to the featured card).
  ...
  // The featured card rendered as its own pseudo-row in the "Featured" group.
    ...
    <div className="grid gap-6 lg:grid-cols-[320px_1fr]">
          ...
          <div className="p-4">
            ...
            {deck.sourceUrl && (
              ...
              >
                ...
                <span>
                  Decklist source{deck.source ? `: ${deck.source}` : ""}
                </span>
              </a>
            )}
            ...
            <div className="mt-4 rounded-lg bg-ink-900 p-3">
              ...
              <div className="text-[11px] text-slate-500">
                {deck.totalCards - 1}-card main deck + featured · {deck.pricedCards}/{deck.totalCards} priced
              </div>
              {deck.sideboardCards > 0 && (
                <div className="mt-1 text-[11px] text-slate-500">
                  + {fmt(deck.sideboardCents)} sideboard ({deck.sideboardCards} cards)
                </div>
              )}
            </div>
      ...
      <div className="min-w-0">
        <div className="card-surface overflow-hidden">
          <div className="border-b border-ink-700 p-4">
            <h2 className="font-bold text-white">
              Decklist <span className="text-slate-500">· {deck.totalCards - 1} cards + featured</span>
            </h2>
          </div>

          <Section title="Featured" items={[legendRow]} />
          {GROUPS.filter((g) => g.title !== "Featured").map((g) => (
            <Section
              key={g.title}
              title={g.title}
          ...
          {deck.sideboardCards > 0 && (
            <div className="flex items-center justify-between border-t border-ink-800 px-4 py-2">
              <span className="text-xs text-slate-500">Sideboard ({deck.sideboardCards} cards)</span>
        ...
        <p className="mt-3 text-center text-[11px] text-slate-600">
          ...
          {" "}price comparison. {SITE_NAME} may earn a commission on some outbound links.
        </p>
      </div>
    </div>
  );
