...
import { buildAboutCard, buildCardFaqs } from "@/lib/card-copy";
...
export default async function CardPage({ params }: { params: { id: string } }) {
  ...
  const [otherPrints, reviewRows, cheaperInSet, sameDomainCards, sameRarityCards, character] = await Promise.all([
    card.nameNormalized
    ...
    // Character hub link. Fetched separately and soft-failed — speciesSlug/
    ...
    prisma.card
      .findUnique({ where: { id: card.id }, select: { characterSlug: true, characterName: true } })
      .catch(() => null as { characterSlug: string | null; characterName: string | null } | null),
  ]);
  ...
  // One Piece prints a card as standard or as a foil/parallel alt-art, with no
  // One Piece-style Holo/Reverse-Holo split — so this is a two-way split on the
  // isFoil flag the importer already sets.
  ...
  // Reverse Holo, then the recompute. Many One Piece chase cards exist ONLY as a
  ...
  const headlineLabel = headlineIsFoil ? "✦ Foil from" : cheapestFoil != null ? "Standard from" : "Cheapest price";
        ...
        <div className="min-w-0">
          <div className="card-surface p-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
              ...
              {card.might != null && <Metric label="Power" value={String(card.might)} />}
        ...
        <section className="mt-8">
          <div className="mb-4 flex items-end justify-between gap-3">
            ...
            {character?.characterSlug && (
              <Link href={`/character/${character.characterSlug}`} className="shrink-0 text-xs font-semibold text-brand-400 hover:underline">
                Full {character.characterName ?? card.name} price guide →
              </Link>
            )}
          </div>
      ...
      {sameDomainCards.length > 0 && (
        <section className="mt-8">
          <div className="mb-4">
            ...
            <p className="mt-0.5 text-xs text-slate-500">
              More {card.domain}-type singles from across the One Piece Card Game, sorted by cheapest price.
            </p>
          </div>
          <div className="-mx-1 flex gap-4 overflow-x-auto px-1 pb-2">
      ...
      {sameRarityCards.length > 0 && (
        <section className="mt-8">
          <div className="mb-4">
            ...
            <p className="mt-0.5 text-xs text-slate-500">
              More {card.rarity.toLowerCase()} singles from across the One Piece Card Game, sorted by cheapest price.
            </p>
          </div>
          <div className="-mx-1 flex gap-4 overflow-x-auto px-1 pb-2">
