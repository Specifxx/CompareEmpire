...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  ...
  const title = `${card.name} (${card.setCode} ${card.collectorNumber}) — car price in ${info.place}`;
  const description = `Compare live ${info.adjective} prices for ${card.name}, car ${card.setName} ${card.collectorNumber}${price}. Find the cheapest store to buy this card in ${info.place}.`;
...
export default async function CardPage({ params }: { params: { id: string } }) {
  ...
  // cars come in both standard and FOIL finishes (same collector number,
  ...
  const jsonLd = {
    ...
    description: `${card.name} — car ${card.setName} (${card.setCode}) ${card.collectorNumber}. Compare ${info.adjective} prices.`,
        ...
        <div className="min-w-0">
          <div className="card-surface p-5">
            ...
            <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
              <Metric label="Lowest listed" value={lowestPrice != null ? fmt(lowestPrice) : "—"} highlight />
              <Metric label="Listings at" value={`${prices.length} ${prices.length === 1 ? "site" : "sites"}`} />
              {card.might != null && <Metric label="Power" value={`${card.might} kW`} />}
              <Metric label="Body" value={card.domain} />
            </div>
          </div>
          ...
          <div className="card-surface mt-6 overflow-hidden">
            ...
            <p className="border-t border-ink-800 p-3 text-center text-[11px] text-slate-600">
              Prices are collected from public store listings and may change. CarCompare
