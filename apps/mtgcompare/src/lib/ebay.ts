...
// Japanese (and Chinese/Korean) Magic printings share card names — and often the
...
// real Magic set catalogue: each set name → a whitespace-tolerant, regex-escaped
...
function numberMatches(title: string, number: string, total: string, setCode: string): boolean {
  ...
  // identity, e.g. Magic promos "SWSH262", "SVP 044", or "XY66"/"BW47". eBay
...
export async function searchEbayLowest(card: {
  ...
  const params = new URLSearchParams({
    ...
    q: `${card.name} ${card.number.replace(/[^0-9]/g, "")}${card.isSignature ? " signature" : ""}${card.isPromo ? " promo" : ""} Magic`,
...
// Keyword each sealed product type must appear as in an eBay title (Magic
