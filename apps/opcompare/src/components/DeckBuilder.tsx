...
export function DeckBuilder({ initialList }: { initialList?: string }) {
  ...
  const marketLabel = country === "NZ" ? " New Zealand" : country === "US" ? " US" : country === "GB" ? " UK" : " Australian";
  const marketShort = country === "NZ" ? "NZ" : country === "US" ? "US" : country === "GB" ? "UK" : "AU";
      ...
      <div className="lg:sticky lg:top-20 lg:self-start">
        <div className="card-surface p-4">
          ...
          <p className="mb-2 text-xs text-slate-500">
            ...
            <span className="font-mono">4 Nami</span>. Set codes like{" "}
            <span className="font-mono">(OP01-003)</span> are optional. (TCGplayer Mass Entry format.)
          </p>
          <textarea
            value={text}
            onChange={(e) => setText(e.target.value)}
            rows={14}
            placeholder={"1 Monkey D. Luffy\n4 Nami\n…"}
        ...
        {loading ? (
          <div className="card-surface grid place-items-center p-16 text-center text-slate-400">
            <div className="flex flex-col items-center gap-3">
              ...
              <p className="text-xs">Matching each card to the cheapest{marketLabel} price.</p>
            </div>
          </div>
        ) : !result ? (
          <div className="card-surface grid place-items-center p-16 text-center text-slate-400">
          ...
          <>
            ...
            <p className="mt-3 text-center text-[11px] text-slate-600">
              Total uses each card&apos;s cheapest in-stock {marketShort} price and may span multiple stores.
            </p>
          </>
        )}
      </div>
