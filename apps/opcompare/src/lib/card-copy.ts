...
export interface CardCopyInput {
  ...
  domain: string; // card colour: Red, Green, Blue, Purple, Black, Yellow
  type: string; // supertype: One Piece, Trainer, Energy
  rarity: string;
...
export function buildAboutCard(seedKey: string, c: CardCopyInput): string {
  const hasPower = c.type === "Leader" || c.type === "Character";
  const hp = hasPower && c.might != null ? `, ${c.might} power` : "";
  const domainClause = c.domain ? ` It's a ${c.domain} card.` : "";
...
export function buildCardFaqs(c: CardCopyInput): CardFaq[] {
  const costAnswer =
    c.headlineCents != null
      ? `The cheapest live price for ${c.name} is ${c.fmt(c.headlineCents)}, across ${c.storeCount} ${c.storeCount === 1 ? "store" : "stores"} OPCompare tracks in ${c.adjective} stores.${
  ...
  const whereAnswer =
    c.headlineCents != null
      ? `You can buy ${c.name} from any of the ${c.storeCount} stores OPCompare compares — the price table above is sorted cheapest first, including postage where we know it.`
      : `OPCompare hasn't matched a live listing for ${c.name} yet — search eBay from this page, or check back soon as our price feeds refresh regularly.`;
  ...
  const printingsAnswer =
    c.otherPrintingsCount > 0
      ...
      : `${c.name} doesn't have any other printings currently tracked on OPCompare.`;
